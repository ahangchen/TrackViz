...
from util.file_helper import read_lines_and, write
from util.file_helper import write_line
from util.serialize import pickle_save

predict_path = 'data/top10/predict_pid.log'
test_path = 'data/top10/test_tracks.txt'
...
renew_pid_path = 'data/top10/renew_pid.log'
renew_score_path = 'data/top10/renew_ac.log'
...
predict_line_idx = 0
predict_track_path = 'data/top10/predict_tracks.txt'
...
def get_predict_tracks():
    ...
    def add_predict_track(line):
        ...
        write_line('data/top10/predict_c%d.txt' % int(camera),
                   ('%04d_c%ds%d_%d_n.jpg' % (int(predict_line_idx) + 1, int(camera), s_num, int(track_time))))
...
def store_sorted_deltas():
    ...
    for camera_delta in camera_delta_s:
        for delta_s in camera_delta:
            ...
            write_line('data/top10/sorted_deltas.txt', per_camera_deltas)
    # for python
    pickle_save('data/top10/sorted_deltas.pickle', camera_delta_s)
...
def interval_scores():
    ...
    pickle_save('data/top10/interval_scores.pickle', camera_pair_travel_probs)
