...
def cross_st_img_ranker():
    ...
    for i, person_ap_pids in enumerate(persons_ap_pids):
        cross_scores = list()
        for j, person_ap_pid in enumerate(person_ap_pids):
            if persons_track_scores[i][j] > 0.7:
                print(real_pids[i] == real_pids[person_ap_pid - 1])
                print('track: %f, appear: %f ' % (persons_track_scores[i][j], persons_ap_scores[i][j]))
            cross_scores.append((persons_track_scores[i][j] ** 1) * (persons_ap_scores[i][j]*1))
